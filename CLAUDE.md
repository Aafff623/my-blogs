...
5. **Commit 门禁**：任务完成**先写 `docs/history/{date}/commit-history.md`，进入 Review**；**禁止**未经用户同意自动 commit。用户同意后再合并提交。详见 [`AGENTS.md`](AGENTS.md) §Commit 攒批。
6. **边界**：Agent 只提交本轮对话自己改动的文件；工作树其他变更（用户线上 `/write` 等）由用户自行维护，**不代提、不混合、不干预**。
7. **术语**：新词 → `CONTEXT.md` → `docs/agents/language.md`。
...
- **Commit 攒批**：任务完成默认终点是 Review，不是 commit；先写 `docs/history/{date}/commit-history.md`，用户同意后再合并提交。详见 `AGENTS.md` §Commit 攒批。
- **文件边界**：Agent 只提交本轮自己改动的文件；工作树其他变更（用户线上 `/write` 等）不代提、不混合、不干预。
